...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + pallet_marketplace_nfts::Config + scale_info::TypeInfo {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_sale(
			...
			let owned = pallet_marketplace_nfts::pallet::Pallet::<T>::account(nft_id, origin.clone());
		...
		pub fn buy(
			...
			let owned = pallet_marketplace_nfts::pallet::Pallet::<T>::account(nft_id, seller.clone());
			...
			pallet_marketplace_nfts::pallet::Pallet::<T>::unchecked_transfer(nft_id, seller.clone(), buyer.clone(), amount);
